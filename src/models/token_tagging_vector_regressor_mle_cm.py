...
from transformers import AutoModel, GPT2Model, AutoConfig, GPT2Config, BertConfig, BertModel, get_linear_schedule_with_warmup #AdamW
import numpy as np
...
from src.utils.torch_metrics_cm import (
...
class TokenTaggingVectorRegressorMLE(LightningModule):
    ...
    def __init__(
        ...
        # For tracking the loss by position - cso
        self.train_loss_metrics = loss_metrics()
        self.val_loss_metrics = loss_metrics()
        self.test_loss_metrics = loss_metrics()
        self.epoch_num = 0

        self.savepath = save_path if save_path is not None else f"./losses_cm/f0_dct_{huggingface_model}"
        ...
        print_num_trainable_params(
            self, model_name=f"TokenTaggingVectorRegressorMLE {huggingface_model}"
        )
    ...
    def step(self, batch: Dict[str, torch.tensor], verbose: bool = False):
        ...
        # These different distributions could be implemented as a function dist = get_distribution(distribution_name, mu, var)
        ...
        log_likelihood = dist.log_prob(labels)
        masked_log_likelihood = log_likelihood * vector_loss_mask
        masked_log_likelihood_loss = torch.sum(masked_log_likelihood, dim=-1)
        ...
        # cso
        #pdb.set_trace()
        masked_nll_batch = -masked_log_likelihood_loss.detach()
        loss_table = defaultdict(list)
        freq_table = defaultdict(int)
        for i, loss_tensor in enumerate(masked_nll_batch):
            nonzero_losses = loss_tensor[torch.where(loss_tensor != 0)]
            num_words = len(nonzero_losses)
            for j, loss in enumerate(nonzero_losses):
                loss_table[(j, (num_words-j)-1)].append(loss.cpu().numpy())
        ...
        return neg_masked_log_likelihood_loss, masked_mae, mu, var, vector_loss_mask, loss_table, freq_table
    ...
    def on_train_start(self):
        ...
        # Force train mode for modules that should be trainable
        self.model.train()
        ...
        print(
            ...
            "regressor=", self.regressor.training,
            "dropout=", self.dropout.training if hasattr(self, "dropout") else None,
        )        
    ...
    def on_test_epoch_end(self):
        ...
        np.save(os.path.join(self.loss_dir, f"test_sum.npy"), self.test_loss_metrics.loss_sum)
        np.save(os.path.join(self.loss_dir, f"test_stderr.npy"), self.test_loss_metrics.loss_stderr)
        np.save(os.path.join(self.loss_dir, f"test_freqs.npy"), self.test_loss_metrics.freqs_sum)
