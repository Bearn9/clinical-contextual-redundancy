...
from src.utils.torch_utils_cm import (
    masked_loss,
    masked_GNLLL,
    masked_GNLLL_batch,
    MLPRegressor,
...
class TokenTaggingRegressorMLE(LightningModule):
    ...
    def __init__(
        ...
        self.epoch_num = 0
        if not os.path.isdir("./losses_duration"):
            os.mkdir("./losses_duration")
    ...
    def step(self, batch: Dict[str, torch.tensor], verbose: bool = False):
        ...
        masked_gaussian_nll_batch = masked_GNLLL_batch(
            input=mu.detach(), target=labels.detach(), var=var.detach(), mask=loss_mask.detach(), loss_fn=self.loss_fn
        )
    ...
    def training_step(self, batch: Dict[str, torch.tensor], batch_idx: int):
        # Safety: ensure Lightning hasn't left the backbone in eval mode
        self.model.train()
        self.regressor.train()
        if hasattr(self, "dropout") and self.dropout is not None:
            self.dropout.train()
        ...
        self.train_loss_metrics.update(loss_table, freq_table) #cso
    ...
    def validation_step(self, batch: Dict[str, torch.tensor], batch_idx: int):
        ...
        loss, preds, var, loss_table, freq_table = self.step(batch)
        self.val_loss_metrics.update(loss_table, freq_table) #cso
    ...
    def test_step(self, batch: Dict[str, torch.tensor], batch_idx: int):
        ...
        loss, preds, var, loss_table, freq_table = self.step(batch)
        self.test_loss_metrics.update(loss_table, freq_table) #cso
