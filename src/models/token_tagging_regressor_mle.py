...
class TokenTaggingRegressorMLE(LightningModule):
    ...
    def __init__(
        ...
        save_path: str = None,
        llama_path: str = None,
        loss_dir: str = "./losses_cm/default",
    ):
        ...
        #self.train_loss_by_position = MeanMetricByPos()
        #self.val_loss_by_position = MeanMetricByPos()
        #self.test_loss_by_position = MeanMetricByPos()
        self.epoch_num = -1

        self.train_loss_metrics = loss_metrics()
    ...
    def on_train_start(self):
        ...
        #self.val_loss_by_position.reset()
        self.train_loss_metrics.reset()
        self.val_loss_metrics.reset()  
        print("Lightning module training:", self.training)
        print("Backbone training:", self.model.training)
        print("Head training:", self.regressor.training)  

    # @profile
    def step(self, batch: Dict[str, torch.tensor], verbose: bool = False):
        # pdb.set_trace()
        #print(len(batch["input_text"]), [torch.sum(sent).item() for sent in batch["loss_mask"]]) #, [torch.sum(sent_labels!=None).item() for sent_labels in batch["original_labels"]])))
        ...
        masked_gaussian_nll_batch = masked_GNLLL_batch(
            input=mu, target=labels, var=var, mask=loss_mask, loss_fn=self.loss_fn
        )

        loss_table = defaultdict(int)
        freq_table = defaultdict(int)
        for i, loss_tensor in enumerate(masked_gaussian_nll_batch):
            nonzero_losses = masked_gaussian_nll_batch[i][torch.where(masked_gaussian_nll_batch[i] != 0)]
            num_words = len(nonzero_losses)
            for j, loss in enumerate(nonzero_losses):
                loss_table[(j, (num_words-j)-1)] += loss.item()
                freq_table[(j, (num_words-j)-1)] += 1
        ...
        return masked_gaussian_nll, mu, var, loss_table, freq_table

    def training_step(self, batch: Dict[str, torch.tensor], batch_idx: int):
        # loss is batch loss, train_loss tracks it over the epoch
        loss, preds, var, loss_table, freq_table = self.step(batch)
        self.train_loss(loss)
        #self.train_loss_by_position(loss_table, freq_table)
        self.train_loss_metrics.update(loss_table, freq_table)
    ...
    def on_train_epoch_end(self):
        #print("TRAIN")
        self.epoch_num += 1
        #np.save(f"./losses/train_sum{self.epoch_num}.npy", self.train_loss_by_position.sum.detach().cpu().numpy())

        #np.save(f"./losses/train_total{self.epoch_num}.npy", self.train_loss_by_position.total.detach().cpu().numpy())
        self.train_loss_metrics.compute()
        np.save(os.path.join(self.loss_dir, f"train_sum{self.epoch_num}.npy"), self.train_loss_metrics.loss_sum)
        np.save(os.path.join(self.loss_dir, f"train_mean{self.epoch_num}.npy"), self.train_loss_metrics.loss_mean)
        np.save(os.path.join(self.loss_dir, f"train_stdev{self.epoch_num}.npy"), self.train_loss_metrics.loss_stdev)
        np.save(os.path.join(self.loss_dir, f"train_stderr{self.epoch_num}.npy"), self.train_loss_metrics.loss_stderr)
        np.save(os.path.join(self.loss_dir, f"train_freqs{self.epoch_num}.npy"), self.train_loss_metrics.freqs_sum)       

        #print(self.train_loss_metrics.loss_mean)
        print(self.train_loss_metrics.freqs_sum)
        self.train_loss_metrics.reset()

        #dataset = self.trainer.datamodule.train_dataloader().dataset
        #self.trainer.datamodule.train_dataloader().dataset.segment_freqs = np.zeros(dataset.max_segment_len)
    ...
    def validation_step(self, batch: Dict[str, torch.tensor], batch_idx: int):
        ...
        self.val_loss(loss)
        #self.val_loss_by_position(loss_table, freq_table)
        self.val_loss_metrics.update(loss_table, freq_table)
    ...
    def on_validation_epoch_end(self):
        ...
        #print("VAL")
        #np.save(f"./losses/val_sum{self.epoch_num}.npy", self.val_loss_by_position.sum.detach().cpu().numpy())
        #np.save(f"./losses/val_total{self.epoch_num}.npy", self.val_loss_by_position.total.detach().cpu().numpy())
        self.val_loss_metrics.compute()
        np.save(os.path.join(self.loss_dir, f"val_sum{self.epoch_num}.npy"), self.val_loss_metrics.loss_sum)
        np.save(os.path.join(self.loss_dir, f"val_mean{self.epoch_num}.npy"), self.val_loss_metrics.loss_mean)
        np.save(os.path.join(self.loss_dir, f"val_stdev{self.epoch_num}.npy"), self.val_loss_metrics.loss_stdev)
        np.save(os.path.join(self.loss_dir, f"val_stderr{self.epoch_num}.npy"), self.val_loss_metrics.loss_stderr)
        np.save(os.path.join(self.loss_dir, f"val_freqs{self.epoch_num}.npy"), self.val_loss_metrics.freqs_sum)
    ...
    def test_step(self, batch: Dict[str, torch.tensor], batch_idx: int):
        ...
        self.test_loss(loss)
        #self.test_loss_by_position(loss_table, freq_table)
        self.test_loss_metrics.update(loss_table, freq_table)
        self.log(
    ...
    def on_test_epoch_end(self):
        #print("TEST")
        #np.save(f"./losses/test_sum{self.epoch_num}.npy", self.test_loss_by_position.sum.detach().cpu().numpy())
        #np.save(f"./losses/test_total{self.epoch_num}.npy", self.test_loss_by_position.total.detach().cpu().numpy())
        self.test_loss_metrics.compute()
        np.save(os.path.join(self.loss_dir, "test_sum.npy"), self.test_loss_metrics.loss_sum)
        np.save(os.path.join(self.loss_dir, "test_mean.npy"), self.test_loss_metrics.loss_mean)
        np.save(os.path.join(self.loss_dir, "test_stdev.npy"), self.test_loss_metrics.loss_stdev)
        np.save(os.path.join(self.loss_dir, "test_stderr.npy"), self.test_loss_metrics.loss_stderr)
        np.save(os.path.join(self.loss_dir, "test_freqs.npy"), self.test_loss_metrics.freqs_sum)
    ...
    def on_epoch_end(self):
        ...
        #self.train_loss_by_position.reset()
        #self.val_loss_by_position.reset()
    ...
    def total_training_steps(self) -> int:
        ...
        print(f"-----Num batches: {self.trainer.accumulate_grad_batches}------")
