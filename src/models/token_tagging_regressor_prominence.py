...
from src.utils.torch_utils import (
    ...
    masked_GNLLL_batch, #cso added
    loss_metrics, #cso added
)
...
class TokenTaggingRegressorMLE(LightningModule):
    ...
    def step(self, batch: Dict[str, torch.tensor], verbose: bool = False):
        ...
        for i, loss_tensor in enumerate(masked_gaussian_nll_batch):
            ...
            for j, loss in enumerate(nonzero_losses):
                ...
                #test by making making samples in batch a particular length for a particular count
    ...
    def on_validation_epoch_end(self):
        ...
        if self.epoch_num > 0:
            ...
            self.trainer.datamodule.val_dataloader().dataset.segment_freqs = np.zeros(dataset.max_segment_len)
    ...
    def on_test_epoch_end(self):
        ...
        np.save(f"./losses/test_sum.npy", self.test_loss_metrics.loss_sum)
        np.save(f"./losses/test_stderr.npy", self.test_loss_metrics.loss_stderr)
        np.save(f"./losses/test_freqs.npy", self.test_loss_metrics.freqs_sum)
