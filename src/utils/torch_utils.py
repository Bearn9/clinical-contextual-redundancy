...
class loss_metrics:
    def __init__(self):
        ...
        self.loss_sum = np.zeros((10, 10))
        self.loss_mean = np.zeros((10, 10))
        self.loss_stdev = np.zeros((10, 10))
        self.loss_stderr = np.zeros((10, 10))
        self.freqs_sum = np.zeros((10, 10))

    def update(self, loss_table, freq_table):
        for pos in loss_table.keys():
            value = loss_table[pos]
            if isinstance(value, (list, tuple, np.ndarray)):
                self.losses[pos].extend(value)
            else:
                self.losses[pos].append(float(value))
            self.freqs[pos] += freq_table[pos]
    ...
    def compute(self):
        ...
        for pos in self.losses.keys():
            self.loss_sum[pos] = np.sum(self.losses[pos])
            self.loss_stdev[pos] = np.std(self.losses[pos])
            self.freqs_sum[pos] = self.freqs[pos]

            if self.freqs_sum[pos] > 0:
                self.loss_mean[pos] = self.loss_sum[pos] / self.freqs_sum[pos]
                self.loss_stderr[pos] = self.loss_stdev[pos] / np.sqrt(self.freqs_sum[pos])
